...
\begin{enumerate}
\item In general the ratio of the diffusion radius of a nucleus (\(a\)) to the neutron wavelength (\(\lambda\)), i.e. \(\frac{a}{\lambda} << 1\).  Under this assumption, derive an approximate expression for the \emph{potential} cross section for \(s\)-wave interactions.
\end{enumerate}
\subsection{References}

...
4. [Hebert 2.1] Assume an elastic and isotropic collision in the CM system.  Neglect the initial velocity of the target nucleus.  
   (a) Obtain the probability density $f(\mu)$ where $f(\mu)d\mu$ is the probability for the deviation cosine ($\cos\theta$) to have a value equal to $\mu$ (within an interval $d\mu$) in the LAB system.  (Hint: first obtain the relation between $\mu=\cos\theta$ and $\cos\phi$ by using the Law of Sines in Figure \ref{fig::scatteringVelLABvsCM}.
   (b) Plot.
...
** Solutions                                                      :noexport:
4. Applying the law of sines to Figure \ref{fig::scatteringVelLABvsCM} gives us
\begin{align}
  \frac{\sin\theta}{v_n'} = \frac{sin(\pi-\varphi)}{V_n'} \;\;.
\end{align}
Rearranging and using a trigonometric identity leads to
\begin{align}
  \sin\theta = -\frac{v_n'}{V_n'} sin(\varphi) \;\;.
\end{align}

We know from the law of cosines (as was shown in the notes) that
\begin{align}
  V_n'^2 = \left[ \frac{A^2+1}{(A+1)^2} + 2 \frac{A}{(A+1)^2}\cos(\varphi) \right] \vec{V}_n^2 \;\;.
\end{align}
We also know that $v_n' = \frac{A}{A+1}V_n$, which leads to 
\begin{align}
  V_n'^2 = \left[ \frac{A^2+1}{A^2} + 2 \frac{1}{A}\cos(\varphi) \right] (\vec{v}_n')^2 \;\;.
\end{align}

Putting these two pieces together leads to
\begin{align}
  \sin^2\theta &= \frac{v_n'}{V_n'} sin^2(\varphi) \\
               &= \frac{1}{\frac{A^2+1}{A^2} + 2 \frac{1}{A}\cos(\varphi)} sin^2(\varphi) \\
               &= \frac{A^2 sin^2(\varphi)}{A^2+1 + 2 A \cos(\varphi)}  \;\;.
\end{align}
We can use trigonometry to recast this expression in terms of $\cos\theta$:
\begin{align}
  \cos\theta &= \pm \sqrt{1-\frac{A^2 sin^2(\varphi)}{A^2+1 + 2 A \cos(\varphi)}}  \;\;.
\end{align}
Care must be taken in selecting the appropriate sign.  By inspecting the diagram, we see that a sign change will occur for $\cos\theta$ when $\frac{A^2 sin^2(\varphi)}{A^2+1 + 2 A \cos(\varphi)}=1$.  Call this critical value $\varphi^*$.  When $\varphi \leq \varphi^*$, the positive square root should be taken; when $\varphi > \varphi^*$, the negative square root should be taken.

Our expression relates $\theta$ and $\phi$, but for this to yield a probability density function it must normalize to one.  It can be confirmed that $\cos\theta$ will take values between -1 and 1.  Thus we should check normalization by integrating over this domain: $\int_{-1}^1 \cos\theta d(\cos\theta) = \int_{-1}^1 \mu d\mu = 1$.  Therefore
\begin{align}
  f(\mu) =
  \begin{cases}
...
\end{align}
This expression $\psi(u,\alpha,\beta)$ is called the /generalized Doppler psi function/.
...
** Problems                                                       :noexport:
1. In general the ratio of the diffusion radius of a nucleus ($a$) to the neutron wavelength ($\lambda$) is much less than one, i.e. $\frac{a}{\lambda} << 1$.  Under this assumption, derive an approximate expression for the /potential/ cross section for \(s\)-wave interactions.
2. Why is 1 eV the cutoff for "thermal" neutrons.
3. Derive Eqn. \ref{eq::dopplerSLBWx}.
4. Plot some resonance stuff...
...
\begin{align}
  ...
  \end{cases}
\end{align}
The fact that the Delta function is a /generalized/ function means that it is only really defined with respect to integration.  Pragmatically this presents no difficulty if it used in probability density functions or other functions that must be integrated to yield physically meaningful results.  It is often used to define functions that exist only at a point because

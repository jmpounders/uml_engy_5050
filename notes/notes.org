...
Finally, if we multiply the angular neutron density by the vector velocity then we get the /angular neutron current/: $\vec{J}(\vec{r},\vec{\Omega},E,t) = \vec{v}(\vec{\Omega},E) n(\vec{r},\vec{\Omega},E,t)$.  Integrating this variable over all angles provides a vector-valued quantity which is usually just called /neutron current/: $\vec{J}(\vec{r},E,t) = \int_{4\pi} \vec{v}(\vec{\Omega},E) n(\vec{r},\vec{\Omega},E,t) d\vec{\Omega}$.  Noting $\vec{v}(\vec{\Omega},E) = \vec{\Omega}v(E)$ permits writing the current in terms of the angular neutron flux.
